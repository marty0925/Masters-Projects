...
SET IMPC_parameter_origin_id = TRIM(IMPC_parameter_origin_id);

-- Remove hidden characters
UPDATE parameter_description
SET IMPC_parameter_origin_id = REPLACE(REPLACE(REPLACE(IMPC_parameter_origin_id, '\r', ''), '\n', ''), '\t', '');
...
#Final query  
select datamain.analysis_id as 'Analysis Id', datamain.pvalue as 'P-Value', all_genes.gene_symbol as 'Gene Symbol',
datamain.mouse_life_stage as 'Mouse Life Stage', datamain.mouse_strain as 'Mouse Strain', datamain.parameter_id as 'Parameter Id',
parameter_description.name as 'Parameter Name', group_parameter.Category as 'Parameter Grouping',
phenotype_procedure.procedure_name as 'Procedure Name', disease_info.DO_disease_name as 'DO Disease Name',
disease_info.OMIM_id as 'OMIM Id', phenotype_procedure.is_mandatory as 'Mandatory Procedure' -- 'As' to rename columns for clarity    
	from dcdmCW.all_genes
	inner join datamain on all_genes.gene_accession_id = datamain.gene_accession_id -- Inner join guarentees retrieved have an associated result/phenotype entry
...
where dcdmCW.all_genes.gene_symbol = 'Larp6'; -- Group 4 Query genes; Fam72a, Larp6, Ap2s1, Tm4sf19
